...
SqlStore.prototype._buildModels = function() {
  ...
  var tableName = self.resourceConfig.tableName || pluralize.singular(self.resourceConfig.resource);
  self.baseModel = self.sequelize.define(self.resourceConfig.resource, modelAttributes, { timestamps: false, tableName: tableName });
...
SqlStore.prototype._joiSchemaToSequelizeModel = function(joiSchema) {
  var model = {
    id: { type: new Sequelize.STRING, primaryKey: true },
...
SqlStore.prototype._fixObject = function(json) {
  ...
  var resourceId = pluralize.singular(self.resourceConfig.resource) + "_id";
